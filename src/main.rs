...
fn two(sel: char) -> io::Result<u64> {
    ...
    line.split(',')
        .for_each(|group| {
            ...
            for v in low..=high {
                if sel == 'a' && two_a_is_invalid(v) {
                    invalid_count += v as u64;    
                }
                else if sel == 'b' && two_b_is_invalid(v) {
                    invalid_count += v as u64;    
                }
            }
        });
...
fn two_b_is_invalid(v: u64) -> bool {
    ...
    for (i, re) in TWO_REGEX_SET.iter().enumerate() {
